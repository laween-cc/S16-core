...
$(build)/img/S16-$(os_version)-floppy.img: $(build)/vbr.bin
	@mkdir -p $(dir $@)
...
# format to fat12
	...
	@sync
# inject volume boot record into sector 0 (preserve JMP instruction, OEM name, BPB, extended BPB and volume name)
